...
<nav class="admin-header navbar navbar-default col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
    ...
    <div class="text-left navbar-brand-wrapper">
        <a class="navbar-brand brand-logo" href="index.html"><img style="height:48px ;" src="{{ URL::asset('assets/images/logo.webp') }}"
                alt=""></a>
    ...
    <ul class="nav navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown mr-30">
            <a class="nav-link nav-pill user-avatar" data-toggle="dropdown" href="#" role="button"
                ...
                <img src="{{ Storage::url('public/' . auth()->user()->image) }}" alt="avatar">
            </a>
            <div class="dropdown-menu dropdown-menu-right">
                ...
                @if (auth('admin')->check())
                    <form method="GET" action="{{ route('admin.changepassword', 'admin') }}">
                    @elseif(auth('admin')->check())
                        <form method="GET" action="{{ route('admin.logout', 'admin') }}">
                        @else
                            <form method="GET" action="{{ route('user.changepassword', 'web') }}">
                @endif
                {{-- @csrf --}}
                <a class="dropdown-item" href="#"
                    onclick="event.preventDefault();this.closest('form').submit();"><i class="bx bx-log-out"></i>
                    الملف الشخصي</a>
                </form>
