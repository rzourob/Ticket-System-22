...
<div class="row">
    <div class="col-md-12">
        <div class="card card-statistics h-100">
            <div class="card-body">
                <form>
                    <h4 style="font-family: 'Cairo', sans-serif">عرض جميع التذاكر </h4>
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table id="requestsAll" class="table table-striped table-bordered p-0" class="table datatable">
                        <thead>
                            <tr>
                                <th>No.</th>
                                <th>رقم التذكرة</th>
...
<script>
    let requestsAllTable = $('#requestsAll').DataTable({
        dom: "tiplr",
        responsive: true,
        serverSide: true,
        processing: true,
        ajax: {
            url: '{{ route('viewDeviceMedical.data') }}',
        },
        columns: [
    ...
    $('#deviceTypes').change(function() {
        $('#requestsAll').dataTable().fnFilter($(this).val(), -7);
    });


    $('#subdepartments').change(function() {
        $('#requestsAll').dataTable().fnFilter($(this).val(), -5);
    });

    $('#departments').change(function() {
        $('#requestsAll').dataTable().fnFilter($(this).val(), -6);
    });
    ...
    $('#fromDate').change(function() {
        ...
        if (toDate.length > 0)
            $('#requestsAll').dataTable().fnFilter($(this).val() + '#' + toDate, -2);
    });

    $('#toDate').change(function() {
        var fromDate = $('#fromDate').val();
        if (fromDate.length > 0)
            $('#requestsAll').dataTable().fnFilter(fromDate + '#' + $(this).val(), -2);
    });
</script>
